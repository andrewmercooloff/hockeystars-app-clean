-- Полное обновление всех упражнений с детальными данными
...
SELECT exercise_id, title_ru, 
       CASE WHEN benefits_ru IS NOT NULL AND jsonb_array_length(benefits_ru) > 0 THEN 'Заполнено' ELSE 'Пусто' END as benefits_status,
       CASE WHEN instructions_ru IS NOT NULL AND jsonb_array_length(instructions_ru) > 0 THEN 'Заполнено' ELSE 'Пусто' END as instructions_status,
       CASE WHEN tips_ru IS NOT NULL AND jsonb_array_length(tips_ru) > 0 THEN 'Заполнено' ELSE 'Пусто' END as tips_status
FROM exercises 
WHERE exercise_id IN ('11', '12', '13', '14', '15', '16', '19', '20', '21', '26')
