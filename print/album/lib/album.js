...
.slot .ghost{position:absolute;left:${-size.bleed}mm;top:${-size.bleed}mm;filter:grayscale(.92) saturate(.25) contrast(.72) blur(.35mm);opacity:.13;}
.slot .ghost::after{content:"";position:absolute;inset:0;pointer-events:none;
  background:linear-gradient(125deg,color-mix(in srgb,var(--secondary) 14%,transparent) 0%,transparent 42%,color-mix(in srgb,var(--secondary) 8%,transparent) 100%);
