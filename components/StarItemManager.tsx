...
export default function StarItemManager({ playerId, isEditing = false, onItemsUpdated }: StarItemManagerProps) {
  ...
  const loadItems = async () => {
    try {
      ...
      Alert.alert(t('common.error'), t('gifts.failedToLoadItems'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const pickImage = async (itemType: 'autograph' | 'stick' | 'puck' | 'jersey', existingItemId?: string) => {
    try {
      ...
      if (currentUser.id !== playerId) {
        Alert.alert(t('common.error'), t('gifts.noPermissionToUpload'));
        return;
      }
      ...
      if (!result.canceled && result.assets[0]) {
        ...
        if (result.assets[0].uri) {
          ...
          Alert.alert(t('common.error'), t('gifts.failedToGetImageUri'));
        }
      } else {
      ...
      Alert.alert(t('common.error'), t('gifts.failedToSelectImage'));
    }
  };
  ...
  const uploadItem = async (imageUri: string, itemType: 'autograph' | 'stick' | 'puck' | 'jersey') => {
    try {
      ...
      // Проверяем, что текущий пользователь совпадает с playerId
      if (currentUser.id !== playerId) {
        throw new Error(t('gifts.noPermissionToUpload'));
      }
      ...
      if (!imageUri || imageUri.trim() === '') {
        throw new Error(t('gifts.emptyImageUri'));
      }
      ...
      if (imageUri.startsWith('file://')) {
        ...
        if (error) {
          ...
          throw new Error(t('gifts.formDataUploadError'));
        }
        ...
        try {
          ...
          if (blob.size === 0) {
            throw new Error(t('gifts.emptyImage'));
          }
          ...
          throw new Error(t('gifts.imageUploadError', { error: fetchError.message }));
        }
      }
      ...
      Alert.alert(t('common.success'), t('gifts.itemUploadedSuccessfully', { itemType: getItemTypeName(itemType) }));
      loadItems();
      onItemsUpdated?.();
    } catch (error) {
      console.error('Error uploading item:', error);
      Alert.alert(t('common.error'), t('gifts.failedToUploadItem'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const updateItemImage = async (itemId: string, imageUri: string) => {
    try {
      ...
      const currentUser = await loadCurrentUser();
      if (!currentUser) {
        throw new Error(t('gifts.userNotAuthorized'));
      }
      
      if (currentUser.id !== playerId) {
        throw new Error(t('gifts.noPermissionToEdit'));
      }

      // Находим существующий подарок
      const existingItem = items.find(item => item.id === itemId);
      if (!existingItem) {
        throw new Error(t('gifts.itemNotFound'));
      }
      ...
      if (imageUri.startsWith('file://')) {
        ...
        if (error) {
          throw new Error(t('gifts.failedToUploadNewImage'));
        }
      ...
      if (updateError) {
        throw new Error(t('gifts.failedToUpdateDatabase'));
      }

      Alert.alert(t('common.success'), t('gifts.itemImageUpdated'));
      ...
      Alert.alert(t('common.error'), t('gifts.failedToUpdateItemImage'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const showAddMenu = () => {
    ...
    if (!existingTypes.includes('autograph')) {
      availableTypes.push({ text: t('gifts.autograph'), onPress: () => pickImage('autograph') });
    }
    if (!existingTypes.includes('stick')) {
      availableTypes.push({ text: t('gifts.stick'), onPress: () => pickImage('stick') });
    }
    if (!existingTypes.includes('puck')) {
      availableTypes.push({ text: t('gifts.puck'), onPress: () => pickImage('puck') });
    }
    if (!existingTypes.includes('jersey')) {
      availableTypes.push({ text: t('gifts.jersey'), onPress: () => pickImage('jersey') });
    }
    
    if (availableTypes.length === 0) {
      Alert.alert(t('gifts.allGiftsAdded'), t('gifts.allGiftTypesAdded'));
      return;
    }
    
    availableTypes.push({ text: t('common.cancel'), style: 'cancel' });
        ...
        { text: t('common.cancel'), style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              onItemsUpdated?.();
              Alert.alert(t('common.success'), t('gifts.itemDeleted'));
            } catch (error) {
              console.error('Error deleting item:', error);
              Alert.alert(t('common.error'), t('gifts.failedToDeleteItem'));
            }
          },
        },
      ]
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>{t('gifts.gifts')}</Text>
      ...
      <ScrollView style={styles.itemsList} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
        {items.length === 0 ? (
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptySubtext}>{t('gifts.clickPlusToAdd')}</Text>
          </View>
        ) : (
          items.map((item) => (
