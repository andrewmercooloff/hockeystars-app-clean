# Full video analysis and fix for App Store
param(
    [string]$VideoFile = "IMG_5198.MOV"
)

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "  VIDEO ANALYSIS AND FIX FOR APP STORE" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
if (-not $ffprobe) {
    ...
    Write-Host "1. winget install ffmpeg" -ForegroundColor Cyan
    Write-Host "2. Or download from https://ffmpeg.org/download.html" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "After installation, RESTART PowerShell and run the script again." -ForegroundColor Yellow
    exit 1
}
...
try {
    ...
    if ($allOk) {
        ...
        Write-Host "Create fixed version of video? (Y/N)" -ForegroundColor Yellow
        $response = Read-Host
        ...
        if ($response -eq 'Y' -or $response -eq 'y') {
            ...
            if (-not $ffmpeg) {
                Write-Host "FFmpeg not found for conversion!" -ForegroundColor Red
                exit 1
            }
            ...
            if ($LASTEXITCODE -eq 0) {
                Write-Host ""
                Write-Host "OK: Video successfully fixed!" -ForegroundColor Green
                ...
                Write-Host ""
                Write-Host "X Error converting video!" -ForegroundColor Red
            ...
            Write-Host ""
            Write-Host "Manual conversion command:" -ForegroundColor Cyan
