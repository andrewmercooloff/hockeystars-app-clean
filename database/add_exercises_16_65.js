...
// Создаем SQL скрипт для добавления упражнений 16-65
let sqlScript = `-- Добавление упражнений 16-65 в базу данных
...
// Обрабатываем упражнения 16-65
Object.keys(ruExercises).forEach(exerciseId => {
  const id = parseInt(exerciseId);
  if (id < 16) return; // Пропускаем упражнения 1-15
...
fs.writeFileSync('./database/add_exercises_16_65.sql', sqlScript);

console.log('✅ SQL скрипт создан: database/add_exercises_16_65.sql');
console.log(`📊 Будет добавлено упражнений: ${Object.keys(ruExercises).filter(id => parseInt(id) >= 16).length}`);
