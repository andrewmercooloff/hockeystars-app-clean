-- Исправление ТОЛЬКО проблем RLS (без Storage)
-- Используйте этот скрипт если Storage не настроен
...
-- 2. Создаем политики для item_requests (основная проблема из Security Advisor)
...
-- 4. Создаем политики для player_museum
...
-- 6. Создаем политики для player_teams
...
-- Готово! Основные проблемы RLS исправлены
-- Storage можно настроить позже через Dashboard
