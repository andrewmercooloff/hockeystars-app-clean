-- Исправление ограничения уникальности для таблицы player_teams
...
-- Для записей с одинаковыми годами добавляем небольшие различия
-- чтобы избежать конфликтов с новым ограничением
-- Используем подзапрос вместо оконной функции
UPDATE player_teams 
SET start_year = start_year + (
  SELECT COALESCE(MAX(sub.counter), 0)
  FROM (
    SELECT 
      player_id, 
      team_id, 
      start_year, 
      end_year,
      ROW_NUMBER() OVER (PARTITION BY player_id, team_id, start_year, end_year ORDER BY created_at) - 1 as counter
    FROM player_teams 
    WHERE start_year IS NOT NULL
  ) sub
  WHERE sub.player_id = player_teams.player_id 
    AND sub.team_id = player_teams.team_id 
    AND sub.start_year = player_teams.start_year 
    AND sub.end_year = player_teams.end_year
    AND sub.counter > 0
)
WHERE EXISTS (
  SELECT 1
  FROM player_teams pt2
  WHERE pt2.player_id = player_teams.player_id
    AND pt2.team_id = player_teams.team_id
    AND pt2.start_year = player_teams.start_year
    AND pt2.end_year = player_teams.end_year
    AND pt2.id != player_teams.id
