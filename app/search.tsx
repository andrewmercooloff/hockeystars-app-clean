...
export default function SearchScreen() {
  ...
  const countries = useMemo(() => {
    ...
    const countriesWithTranslations = rawCountries.map(country => {
      ...
      const translated = t(`countries.${country.toLowerCase()}`);
      
      // Если перевод не найден, используем оригинальное название
      const displayName = translated !== `countries.${country.toLowerCase()}` ? translated : country;
        ...
        <View style={styles.playerDetails}>
          ...
          {item.status !== 'admin' && (
            <Text style={styles.playerInfo} numberOfLines={2} ellipsizeMode="tail">
              {t(`countries.${item.country?.toLowerCase()}`) || item.country} | {t(`profile.${item.position}`) || item.position} | {formatBirthDate(item.birthDate || '')} | {item.height} {t('profile.cm')} | {item.weight} {t('profile.kg')} | {(() => {
